...
for mes in range(nM):
    for aceite in range(nA):
        s.add_soft(refinado[mes][aceite] == 0, 1, "usoAceite")

## EXTENSIONES

# constraint forall (mes in 1..nM) (count (aceite in 1..nA) (refinado[mes, aceite] > 0) >= K[mes])
for mes in range(nM):
    s.add(addSum([If(val > 0, 1, 0) for val in refinado[mes]]) >= arrayK[mes])
...
for mes in range(nM):
    for aceite in range(nA):
        s.add(
            Or(
                refinado[mes][aceite] == 0,
                refinado[mes][aceite] >= matrixT[mes][aceite],
            )
        )
...
for mes in range(nM):
    for aceite1 in range(nA):
        for aceite2 in range(aceite1 + 1, nA):
            s.add(
                Or(
                    incompatible[aceite1][aceite2] == 0,
                    refinado[mes][aceite1] == 0,
                    refinado[mes][aceite2] == 0,
                )
            )
...
## EXTENSIONES ADICIONALES
for mes in range(nM):
    s.add(addDotProd(comprado[mes], precios[mes]) <= maxG[mes])
